import type { Category, CharacterStats, MatchupEntry } from './types';

export function computeStats(matchups: MatchupEntry[]): CharacterStats | null {
...
export function getCategory(score: number): Category {
  if (score >= 6.2) { return 'strong-adv'; }
  if (score >= 5.6) { return 'decent-adv'; }
  if (score >= 5.2) { return 'small-adv'; }
  if (score >= 4.8) { return 'even'; }
  if (score >= 4.4) { return 'small-dis'; }
  if (score >= 3.8) { return 'decent-dis'; }
  return 'strong-dis';
}
...
const OPPONENT_NAME_MAP: Record<string, string> = {
  'simon':            'simon / richter',
...
export function findMatchup(matchups: MatchupEntry[], opponentName: string): MatchupEntry | undefined {
...
export function getPercentile(matchups: MatchupEntry[], score: number): number {
...
export function getRank(matchups: MatchupEntry[], score: number): { rank: number; total: number } {
